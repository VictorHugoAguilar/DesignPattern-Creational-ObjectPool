...
public abstract class AbstractObjectPool<T extends IPooledObject> implements IObjectPool<T> {
	...
	private static class PooledObjectStatus<T> {
		...
		public PooledObjectStatus(T pooledObect) {
			this.used = false;
			this.uuid = UUID.randomUUID();
			this.pooledObject = pooledObect;
		}
	}
	...
	private T getInternalObject() throws PoolException {
		...
		synchronized (freeStack) {
			if (!freeStack.isEmpty()) {
				...
				first.used = true;
				System.out.println("Provisioniing 1 Object > " + first.uuid.toString());
				useStack.push(first);
			...
			synchronized (fullStack) {
				if (fullStack.size() < maxInstances) {
					...
					System.out.println("Provisioniing 2 Object > " + returnObject.uuid.toString());
		...
		do {
			...
			synchronized (freeStack) {
				if (!freeStack.isEmpty() && !freeStack.peek().used) {
					...
					returnObject.used = true;
					useStack.push(returnObject);
					System.out.println("Provisioniing 3 Object > " + returnObject.uuid.toString());
					return returnObject.pooledObject;
				}
			}
	...
	private PooledObjectStatus<T> createNewPooledObject() {
		...
		System.out.println("new Pooleable Object UUID > " + pooled.uuid.toString() + " poolSize = " + fullStack.size());
		return pooled;
	}
		...
		for (PooledObjectStatus<T> item : this.fullStack) {
			if (item.pooledObject == pooledObject) {
				if (pooledObject.validate()) {
					...
					item.used = false;
					System.out.println("Object return > " + item.uuid.toString());
					return;
				} else {
	...
	public String toString() {
		return "AbstractObjecftPool => currentSize > " + fullStack.size() + "  free  > " + freeStack.size()
				+ " used >  " + useStack.size();
	}
